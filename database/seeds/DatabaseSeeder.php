<?php

use Illuminate\Database\Seeder;

class DatabaseSeeder extends Seeder
{
    /**
     * Seed the application's database.
     *
     * @return void
     */
    public function run()
    {
        // $this->call(UserSeeder::class);
        DB::table('managers')->insert([         //管理员
            [
                'name' => '我是超人001',
                'stu_id' => 'Sky31',
                'password' => md5('Sky31666'),
                'department' => '技术开发部',
                'level' => 0,
            ],
            [
                'name' => '张贵妇',
                'stu_id' => '201905190401',
                'password' => md5('Sky31666'),
                'department' => '技术开发部',
                'level' => 0,
            ],
        ]);

        DB::table('users')->insert([            //用户
            [
                'nickname' => '啦啦啦',
                'name' => '张贵妇',
                'stu_id' => '201905194444',
                'password' => bcrypt('Sky31666'),
                'collection' => '[]',
                'like' => '[]',
                'eatest' => '[]',
                'countdown' =>'[]',
                'avatar' => '"http://zgf.sky31.cn:10302/storage/avatar/avatar.jpg"',
                'focus' => 0,
                'focused' => 0,
                'remember' => md5('Sky31666' . time() . rand(1000, 2000)),
            ],
            [
                'nickname' => '啦啦啦',
                'name' => '章子烨',
                'stu_id' => '201905190416',
                'password' => bcrypt('zzy20011218'),
                'collection' => '[]',
                'like' => '[]',
                'eatest' => '[]',
                'countdown' =>'[]',
                'avatar' => '"http://zgf.sky31.cn:10302/storage/avatar/avatar.jpg"',
                'focus' => 0,
                'focused' => 0,
                'remember' => md5('zzy20011218' . time() . rand(1000, 2000)),
            ],
            [
                'nickname' => '啦啦啦',
                'name' => '李蓟洋',
                'stu_id' => '201905555537',
                'password' => bcrypt('010420yang'),
                'collection' => '[]',
                'like' => '[]',
                'eatest' => '[]',
                'countdown' =>'[]',
                'avatar' => '"http://zgf.sky31.cn:10302/storage/avatar/avatar.jpg"',
                'focus' => 0,
                'focused' => 0,
                'remember' => md5('010420yang' . time() . rand(1000, 2000)),
            ],
            [
                'nickname' => '啦啦啦',
                'name' => '谢磊',
                'stu_id' => '201905710929',
                'password' => bcrypt('xl20010714'),
                'collection' => '[]',
                'like' => '[]',
                'eatest' => '[]',
                'countdown' =>'[]',
                'avatar' => '"http://zgf.sky31.cn:10302/storage/avatar/avatar.jpg"',
                'focus' => 0,
                'focused' => 0,
                'remember' => md5('xl20010714' . time() . rand(1000, 2000)),
            ]
        ]);

        DB::table('evaluations')->insert([          //评测
            [
                'publisher' => 1,
                'nickname' => '啦啦啦',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 3,
                'collections' => 4,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 2,
                'nickname' => '噜噜噜',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 3,
                'nickname' => '略略略',
                'title' => 'hahaha',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hello',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 4,
                'nickname' => '啦啦啦',
                'title' => 'hahaha',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hello',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 5,
                'nickname' => '噜噜噜',
                'title' => 'hahaha',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hello',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 1,
                'nickname' => '略略略',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 5,
                'collections' => 3,
                'like' => 3,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 2,
                'nickname' => '啦啦啦',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 3,
                'nickname' => '噜噜噜',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 4,
                'nickname' => '略略略',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 5,
                'nickname' => '啦啦啦',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 6,
                'nickname' => '噜噜噜',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 0,
                'like' => 0,
                'status' => 0,
                'img' => '{"1":1}',
            ],
            [
                'publisher' => 1,
                'nickname' => '略略略',
                'title' => 'hello',
                'label' => '{"1":1}',
                'topic' => '螺蛳粉',
                'content' => 'hahaha',
                'top' => 0,
                'score' => 1,
                'views' => 0,
                'collections' => 2,
                'like' => 5,
                'status' => 0,
                'img' => '{"1":1}',
            ],
        ]);

        DB::table('eatest_comments')->insert([          //评论
            'eatest_id' => 1,
            'toId' => 1,
            'fromId' => 1,
            'status' => 0,
            'fromName' => '啦啦啦',
            'fromAvatar' => 'hah',
            'content' => '你好',
            'status' => 1,
            'like' => 3,
            'handleStatus' => 0,
        ]);

        DB::table('courses')->insert([              //自定义课表
            [
                'uid' => 201905190401,
                'course' => '高数',
                'location' => '学活209',
                'teacher' => '张贵妇',
                'week' => json_encode([1,2,3]),
                'week_string' => '1-3周',
                'section_start' => 1,
                'end_start' => 2,
                'section_length' => 2,
                'day' => 1,
            ],
            [
                'uid' => 201905190401,
                'course' => '线代',
                'location' => '学活209',
                'teacher' => '张贵妇',
                'week' => json_encode([1,2,3]),
                'week_string' => '1-3周',
                'section_start' => 3,
                'end_start' => 4,
                'section_length' => 2,
                'day' => 1,
            ],
            [
                'uid' => 201905190403,
                'course' => 'C语言',
                'location' => '学活209',
                'teacher' => '张贵妇',
                'week' => json_encode([1,2,3]),
                'week_string' => '1-3周',
                'section_start' => 5,
                'end_start' => 6,
                'section_length' => 2,
                'day' => 1,
            ],
            [
                'uid' => 201905190402,
                'course' => 'Python',
                'location' => '学活209',
                'teacher' => '张贵妇',
                'week' => json_encode([1,2,3]),
                'week_string' => '1-3周',
                'section_start' => 9,
                'end_start' => 11,
                'section_length' => 2,
                'day' => 1,
            ]
        ]);

        DB::table('course_groups')->insert([        //小组
            'groupName' => '技术开发部',
            'memberSum' => 3,
            'member' => json_encode(['201905190401','201905190402','201905190403']),
            'Founder' => '张贵妇',
            'FounderUid' => '201905190401',
            'sharingCode' => 'SKY314255',
        ]);

        DB::table('info')->insert([            //学生信息
            [
                'sid' => '201905190401',
                'name' => '张贵妇',
                'sex' => '男',
                'college' => '公共管理学院',
                'major' => '信息管理与信息系统',
                'class' => '二班',
            ],
            [
                'sid' => '201905190402',
                'name' => '周舟洲',
                'sex' => '男',
                'college' => '公共管理学院',
                'major' => '信息管理与信息系统',
                'class' => '二班',
            ],
            [
                'sid' => '201905190403',
                'name' => '李以为',
                'sex' => '男',
                'college' => '公共管理学院',
                'major' => '信息管理与信息系统',
                'class' => '三班',
            ],
        ]);

//        DB::table('eatest_reports')->insert([        //举报
//            'eatestId' => '1',
//            'userName' => 'hahaha',
//            'targetName' => 'hahaha',
//            'type' => 'eatest评论举报',
//            'describe' => 'hahaha',
//            'reason' => 'hahaha',
//            'prove' => '[1,2,3]',
//            'status' => 0
//        ]);
    }
}
